...
checks = {
    CHECK_PID_COMMAND                         	: 'pgrep -u $LOGNAME httpd',
    STOP_COMMAND                        	: 'source $HOME/.bash_profile ; /etc/rc.d/init.d/httpd stop',
    PKILL_COMMAND                           	: 'pkill -u $LOGNAME httpd',
    KILL_COMMAND                            	: 'kill -9 $( pgrep -u $LOGNAME httpd ) 2> /dev/null',
...
def shutdown( topology_params, function_params, ssh, response, *args, **kwargs ):
     ...
     return cloudmgrws.ssh_tools.process_steps(
         [
             ...
             {
                 cloudmgrws.ssh_tools.STEP_NAME         : STOP_COMMAND,
                 cloudmgrws.ssh_tools.SHELL_COMMAND     : checks[ STOP_COMMAND ].format( **function_params._asdict() ),
                 cloudmgrws.ssh_tools.TESTS             : [
                                                           {
                                                            cloudmgrws.ssh_tools.TEST_NAME              : cloudmgrws.ssh_tools.IS_RETURN_CODE_INCORRECT,
